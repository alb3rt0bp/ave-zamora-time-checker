...
def create_metrics_table():
    """Crea la tabla DynamoDB de métricas precalculadas (clave simple pk, sin TTL) con el mismo esquema que el template."""
    dynamodb = boto3.resource("dynamodb", region_name=aws_env.AWS_REGION)
    table = dynamodb.create_table(
        TableName=aws_env.DYNAMODB_METRICS_TABLE_NAME,
        AttributeDefinitions=[
            {"AttributeName": "pk", "AttributeType": "S"},
        ],
        KeySchema=[
