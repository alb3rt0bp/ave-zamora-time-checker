...
from gtfsrt_client import GtfsRtClient, TRIP_UPDATES_URL
...
class TestFetch(unittest.TestCase):
    ...
    def test_returns_raw_bytes_on_success(self, mock_urlopen):
        mock_urlopen.return_value = FakeHTTPResponse(b'{"entity": []}')

        result = self.client._fetch(TRIP_UPDATES_URL)

        self.assertEqual(result, b'{"entity": []}')

    @patch("urllib.request.urlopen", side_effect=raise_http_error)
    def test_reraises_http_error(self, mock_urlopen):
        with self.assertRaises(urllib.error.HTTPError):
            self.client._fetch(TRIP_UPDATES_URL)
    ...
    def test_uses_configured_timeout(self, mock_urlopen):
        ...
        client = GtfsRtClient(SAMPLE_LOG_EXTRA, timeout_seconds=7)

        client._fetch(TRIP_UPDATES_URL)
