...
class TestSanitizeRetraso(HandlerTestCase):
    def test_returns_unchanged_when_within_threshold(self):
        ...
        result = self.handler._sanitize_retraso("G100", "Galicia", -10, "08:30", TODAY, NOW, SAMPLE_LOG_EXTRA)

        self.assertEqual(result, -10)
        self.assertEqual(self.get_published_data_quality_alerts(), [])

    def test_returns_unchanged_for_positive_delay(self):
        result = self.handler._sanitize_retraso("G100", "Galicia", 20, "08:30", TODAY, NOW, SAMPLE_LOG_EXTRA)
    ...
    def test_corrects_anomalous_negative_delay(self):
        ...
        result = self.handler._sanitize_retraso("G100", "Galicia", -562, "08:30", TODAY, NOW, SAMPLE_LOG_EXTRA)
