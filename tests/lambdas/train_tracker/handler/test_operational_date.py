...
class TestOperationalDate(HandlerTestCase):
    """
    NIGHT_TAIL_WINDOW_HOURS por defecto = 2 (ver handler.py): [00:00, 02:00)
    sigue perteneciendo operativamente al día calendario anterior.
    """
    ...
    def test_end_of_tail_window_still_belongs_to_previous_day(self):
        next_day = MONDAY + timedelta(days=1)
        self.assertEqual(self.handler._operational_date(_at(next_day, 1, 59)), MONDAY)

    def test_at_tail_window_boundary_matches_calendar_date_again(self):
        next_day = MONDAY + timedelta(days=1)
        self.assertEqual(self.handler._operational_date(_at(next_day, 2, 0)), next_day)
