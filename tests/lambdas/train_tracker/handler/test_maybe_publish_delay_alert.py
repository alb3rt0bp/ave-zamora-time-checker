...
class TestMaybePublishDelayAlert(HandlerTestCase):
    def test_does_not_publish_at_threshold(self):
        ...
        self.handler._maybe_publish_delay_alert(SCHEDULED, 15, "08:45", TODAY, NOW, SAMPLE_LOG_EXTRA)

        self.assertEqual(self.get_published_delay_alerts(), [])

    def test_does_not_publish_below_threshold(self):
        self.handler._maybe_publish_delay_alert(SCHEDULED, 5, "08:35", TODAY, NOW, SAMPLE_LOG_EXTRA)

        self.assertEqual(self.get_published_delay_alerts(), [])

    def test_publishes_above_threshold(self):
        self.handler._maybe_publish_delay_alert(SCHEDULED, 16, "08:46", TODAY, NOW, SAMPLE_LOG_EXTRA)

        alerts = self.get_published_delay_alerts()
        self.assertEqual(len(alerts), 1)

    def test_publishes_expected_payload(self):
        self.handler._maybe_publish_delay_alert(SCHEDULED, 20, "08:50", TODAY, NOW, SAMPLE_LOG_EXTRA)
    ...
    def test_flagship_train_publishes_even_with_zero_delay(self):
        self.handler._maybe_publish_delay_alert(FLAGSHIP_SCHEDULED, 0, "08:56", TODAY, NOW, SAMPLE_LOG_EXTRA)

        alerts = self.get_published_delay_alerts()
        self.assertEqual(len(alerts), 1)

    def test_flagship_train_payload_marks_es_tren_madrugador(self):
        self.handler._maybe_publish_delay_alert(FLAGSHIP_SCHEDULED, 20, "09:16", TODAY, NOW, SAMPLE_LOG_EXTRA)
